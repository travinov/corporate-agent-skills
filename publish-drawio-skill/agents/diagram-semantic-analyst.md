...
Do not discover, search for, or select repository specifications. OpenSpec material is
ordinary document content only when the user explicitly supplied that document. If
an explicitly supplied user document conflicts with current user intent, produce one
consolidated conflict for human decision.
