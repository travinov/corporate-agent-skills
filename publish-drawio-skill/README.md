...
Roadmap и git-flow используют versioned v1 schemas и дополнительный
...
unzip drawio-skill-corporate.zip -d ~/.gigacode/skills
```
...
Поддерживаются `PyYAML>=6.0,<7` и `jsonschema>=4.18,<5`. Проверка registry
использует только уже настроенный pip source и ничего не устанавливает.
