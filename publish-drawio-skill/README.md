...
Короткие пользовательские команды остаются основным интерфейсом:
...
Перед первым mutable run Host выполняет bounded interactive intake. Он
возвращает `status: awaiting_input` и ровно один host-owned вопрос за ход,
сохраняет стабильный `intake_id` и принимает только replay-команду из
`selection_required.replay`. Последовательно задаются не более трёх
блокирующих вопросов; остаток консолидируется в один вопрос, а допущения
применяются только после явного подтверждения. На каждую replay-команду
используйте выданные Host `--intake-id` и `--intake-answer`; не конструируйте
эти скрытые флаги вручную.

Layout Engine v1 строит versioned `layout-request.v1`/`layout-result.v1`.
`layout_backend: auto` сначала допускает только доказанный локальный Node и
vendored `elkjs@0.11.1`, затем при любой ограниченной ошибке ELK использует
обязательный `python-layered` fallback. `layout_backend: python` вообще не
ищет Node, а `layout_backend: elk` fail-closed, если Node/bridge не доказан.
Каждая стратегия ограничена и не повторяется с тем же request/strategy/options
key. `/drawio:trace` показывает `backend_requested`, `backend_selected`,
`node_executable`, `node_version`, `elkjs_version`, `strategy_id`,
`effective_options`, `request_sha256`, `result_sha256`, `fallback_reason`,
stdout/stderr paths и hashes. Node является только optional accelerator:
runtime никогда не запускает `npm`, `npx`, registry lookup, `curl` или иной
сетевой/package resolver.
