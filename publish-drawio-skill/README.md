...
Папка содержит native-compatible manifest `gemini-extension.json` и четыре
описания в `agents/*.md`. На корпоративном ноутбуке используйте GigaCode CLI,
а не локальный Gemini CLI. В корне репозитория поставляются безопасные скрипты
установки, проверки и отката:
...
Перед установкой он проверяет checksum и обязательные файлы ZIP, запускает
...
Для офлайн-режима перенесите три `scripts/gigacode/*.sh`, ZIP и `.zip.sha256`.
Откат: `scripts/gigacode/rollback_drawio_agent_extension.sh --latest`.
