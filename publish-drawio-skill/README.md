...
также выбирает единственный файл. `resume` выбирает единственный run, ожидающий
пользователя. При неоднозначности работа агентов не начинается, а команда
перечисляет варианты.
...
Поле Supervisor `required_roles` выбирает следующие sibling-роли. Уже
выполнившийся и подтверждённый Supervisor сохраняется host-ом в workflow
автоматически, даже если модель не повторила `supervisor` в массиве. Исходный
JSON остаётся в evidence; отсутствующие Semantic Analyst, Repair или Reviewer
автоматически не добавляются и по-прежнему вызывают fail-closed проверку.
