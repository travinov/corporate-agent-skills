...
исправимой попытки. Strict-failed кандидат не является строгим успехом, но
после bounded exhaustion может быть возвращён как `best_effort_completed`,
если Host подтвердил XML, hash/receipt integrity, отсутствие структурных и
семантических ошибок и классифицировал остаток только как layout/readability.
...
## Контракт `1.25.0-corporate.1`

Нормальный пользовательский запуск остаётся коротким:

```text
/drawio:create "Покажи обработку заказа от оплаты до доставки"
/drawio:improve "Убери пересечения стрелок и сохрани смысл"
```

Create/improve сначала выполняет bounded interactive intake. При
`status: awaiting_input` GigaCode должен показать ровно один вопрос из
`selection_required.question` и повторить только готовую команду из
`selection_required.replay`. Host задаёт не более трёх последовательных
блокирующих вопросов, затем консолидирует остаток; assumptions применяются
только после явного согласия. `--intake-id`, `--intake-answer` и
`--accept-intake-assumptions` не вводите вручную до того, как Host вернул их
в replay.

Строгий успех имеет `strict_passed: true`, hash-bound validation receipt,
Reviewer approve и разрешённую publication transaction. Значение
`best_effort_completed` всегда имеет `strict_passed: false`: файл может быть
безопасным и полезным, но оставшиеся layout/readability findings нужно показать
пользователю. `awaiting_human`, `approved_with_findings`, `manual_handoff`,
`stopped` и `error` также не называйте strict success.

После распаковки докажите optional Node и локальный vendored ELK без npm:

```bash
cd "$HOME/Downloads/drawio-skill" && node --version && node scripts/elk_runner.mjs --probe
```

Ожидается JSON с `bridge: drawio-elk-runner` и
`elkjs_version: 0.11.1`. Node не является обязательной зависимостью. Forced
Python fallback проверяется без запуска Node:

```bash
cd "$HOME/Downloads/drawio-skill" && python3 -m unittest tests.test_layout_backend.LayoutBackendTests.test_python_mode_and_sanitized_path_do_not_require_node
```

В `/drawio:trace` и layout evidence проверяйте
`backend_requested`, `backend_selected`, `node_executable`, `node_version`,
`elkjs_version`, `strategy_id`, `effective_options`, `options_sha256`,
`request_sha256`, `result_sha256`, `fallback_reason`, stdout/stderr paths,
capture flags и SHA-256. Для ELK нужны доказанные Node/bridge и
`schema_valid: true`; для автоматического Python fallback нужна явная причина.

Во втором Terminal можно read-only следить за состоянием текущего запуска:

```bash
cd "/Users/travinov-sv/Documents/DrawioTest" && RUN="$(ls -td "$PWD"/.diagram-runs/* 2>/dev/null | head -1)" && echo "RUN=$RUN" && tail -F "$RUN/run-manifest.jsonl"
```

После terminal status (`completed`, `best_effort_completed`,
`approved_with_findings`, `manual_handoff`, `stopped` или `error`) остановите
`tail` через `Ctrl+C` и отдельно проверьте evidence:

```bash
cd "/Users/travinov-sv/Documents/DrawioTest" && EXT="$HOME/.gigacode/extensions/publish-drawio-skill" && RUN="$(basename "$(ls -td "$PWD"/.diagram-runs/* | head -1)")" && python3 "$EXT/scripts/diagram_orchestrator.py" trace --workspace "$PWD" --run "$RUN"
```

ZIP уже содержит Python runtime, схемы, `scripts/elk_runner.mjs`,
`vendor/elkjs/elk.bundled.js`, `LICENSE` и `NOTICE.json`. В runtime запрещены
`npm`, `npx`, registry lookup, `curl` и сетевое получение layout-кода.
Установка, verifier, SHA-256 и rollback полностью описаны в разделах 1–3 и 22;
они выполняются из перенесённого ZIP без GitHub.
...
Ожидаемо: `"version": "1.25.0-corporate.1"`.
...
- Repair — `vllm/MiniMax-M3-113k`, только когда есть применимые findings;
  при доказанном timeout/turn-limit/model-unavailable/empty-response — ровно
  один fallback на `vllm/Qwen3.6-35B-262k` с тем же input SHA-256;
...
Если `host-result.json` всё же вернул semantic/final/unsafe-output checkpoint,
...
Команда должна сама пройти bounded Validator → Repair → Validator цикл.
Обычный route/layout plateau теперь автоматически заканчивается
`best_effort_completed`; `next_commands.continue` используйте только при
реальном semantic conflict или unsafe-output checkpoint. Затем проверьте:
...
нормализованная сигнатура или общий лимит запускает best-effort классификацию.
Ожидайте `status: best_effort_completed`, `strict_passed: false`, непустой
`final_artifact` и список `remaining_findings`, если все ошибки безопасны.
...
## 22. Terminal: откат на сохранённую `1.24.0-corporate.5`
...
Ожидается восстановленная версия `1.24.0-corporate.5` и зарегистрированный
