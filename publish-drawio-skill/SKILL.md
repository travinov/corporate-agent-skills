...
Interpret Supervisor `result.required_roles` as downstream sibling selection.
The deterministic host always retains the already completed Supervisor in its
own workflow bookkeeping, even when the role JSON omits `supervisor`. Preserve
the original role JSON as evidence and never use this self-normalization to add
an omitted Semantic Analyst, Repair, or Reviewer; mandatory sibling and phase
