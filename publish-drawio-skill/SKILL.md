...
/drawio:improve
/drawio:improve "optional additional requirements or corrections"
...
read-only review, then falls back to the only root-level `.drawio`; review
auto-selects only one root-level `.drawio`;
resume auto-selects only one pending checkpoint; trace selects the latest run.
Qwen custom-command arguments cross the shell boundary only through the single
`DRAWIO_COMMAND_ARGS={{args}}` transport value. `command_ux.py` tokenizes that
value without shell evaluation, normalizes a leading `@` only on a `.drawio`
path, and rejects attempts to override the host workspace or CLI. Every
user-visible `next_commands` value must round-trip through this same grammar.
If improve has no explicit request, the deterministic host supplies the standard
intent to fix current Validator and Reviewer findings and records that source.
Stale review handoffs are ignored. Ambiguity fails before role or validator
execution. Explicit flags remain supported for automation.
...
present-but-empty MCP-server allowlist that removes every globally configured
MCP server before discovery, a fork/MCP deny list, default non-interactive
approval, and a bounded turn budget. Treat `--allowed-mcp-server-names` as a
required corporate CLI capability and fail before role execution when it is
unavailable.
...
`supervisor_declared_roles`. The deterministic host owns lifecycle topology:
initial create/improve authorizes Supervisor, Semantic Analyst, Repair, and
Reviewer; continuation authorizes Supervisor, Repair, and Reviewer. Record this
set as `host_mandatory_roles` and their union as effective `required_roles`.
Repair runs only when deterministic validation or Reviewer findings require it.
`host-result.json` and `/drawio:trace` expose the same split under `role_policy`.
...
**Step -1 — Diagram Intake Agent.** For broad, ambiguous, or non-trivial requests, read `references/diagram-intake.md` before choosing a generator. Classify the likely diagram type, determine what is already known, and ask only blocking semantic or topological questions. Questions come from a matrix, not a fixed questionnaire. Ask one question per turn and at most three blocking questions; after that, consolidate remaining gaps or require explicit acceptance of host-recorded assumptions. Explicit and preserved existing types skip confirmation, ambiguous types use native selection, and non-blocking visual choices become assumptions. Do not allocate a lifecycle run until the host reports the confirmed diagram brief complete.
