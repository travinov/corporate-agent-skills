...
Artifact validation SHALL emit the same machine-readable finding envelope used by source validators, with artifact-specific layers and stable codes, and SHALL support strict promotion of warnings without changing finding identity.
...
- **THEN** validation exits successfully while preserving the warnings in the report

#### Scenario: Artifact has warnings in strict mode
- **WHEN** the same artifact is validated in strict mode
- **THEN** warnings are promoted to errors, validation exits non-zero, and finding codes and paths remain unchanged
